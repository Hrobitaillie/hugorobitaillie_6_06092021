export function filtering(allPhotographers){
    ...
    function filterPhotographs(){
        for( let photographCount = 0; photographCount < allPhotographers.length ; photographCount++){
            let actualCard = document.getElementById("card-"+allPhotographers[photographCount].id);
            if (allPhotographers[photographCount].tags.filter(valeur => active.includes(valeur))!="" || active.length===0){
